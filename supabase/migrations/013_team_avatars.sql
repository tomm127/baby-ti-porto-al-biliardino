-- BTPB
-- Migration 013: team profile images / avatars
-- Intended after the emergency-pause snapshot migration and current admin migrations.
...
-- Public images: team logos/photos are deliberately visible to every tournament viewer.
...
create or replace function public.admin_set_team_avatar(
  p_team_id uuid,
  p_avatar_url text
)
...
revoke all on function public.admin_set_team_avatar(uuid, text) from public;
grant execute on function public.admin_set_team_avatar(uuid, text) to authenticated;


-- Snapshot replacement: same payload currently used by the app, with avatar_url
-- added to every team. Emergency pause fields are intentionally preserved.
...
begin
  ...
  select jsonb_build_object(
    ...
    'settings', (
      select to_jsonb(x) from (
        select s.tournament_id, s.ordering_mode, s.qualifiers_per_group,
               ...
               s.team_pin_enabled, s.emergency_paused, s.emergency_paused_at
    ...
    'teams', coalesce((
      ...
      from (
        select tm.id, tm.tournament_id, tm.name, tm.status, tm.avatar_url
